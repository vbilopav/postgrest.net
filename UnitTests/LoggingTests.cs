    ...
    public class LoggingTests : PostgRestClassFixture<LoggingTests.Services, LoggingTests.LifeCycle>
    {
        ...
        public async Task TestLogWriteFromPostgresFunction()
        {
            ...
            (output as TestOutputHelperAdapter).Inject(message =>
            {
                ...
                if (!errorFound)
                {
                    errorFound = string.Equals(message, "[Error] PostgRest.net.PgDataContentService [0] ERROR\nMessage: P0001: error log\nDetail: \nLine: 3778\nInternalPosition: 0\nPosition: 0\nSqlState: P0001\nStatement: select rest__get_test_logging_error()\nColumnName: \nConstraintName: \nTableName: \nInternalQuery: \nWhere: PL/pgSQL function rest__get_test_logging_error() line 3 at RAISE\nHint: \n\n");
                }
            });

            await RestClient.GetAsync<object>("https://localhost:5001/api/test-logging-info");
            await RestClient.GetAsync<object>("https://localhost:5001/api/test-logging-notice");
            await RestClient.GetAsync<object>("https://localhost:5001/api/test-logging-log");
            await RestClient.GetAsync<object>("https://localhost:5001/api/test-logging-warn");
            await RestClient.GetAsync<object>("https://localhost:5001/api/test-logging-debug");
            await RestClient.GetAsync<object>("https://localhost:5001/api/test-logging-error");
